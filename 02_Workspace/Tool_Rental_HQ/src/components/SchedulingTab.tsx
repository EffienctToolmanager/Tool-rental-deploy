...
export const SchedulingTab: React.FC<SchedulingTabProps> = ({ assets, isAdmin, onRefreshAssets }) => {
  ...
  const [formStage, setFormStage] = useState<'active_rental' | 'calibration' | 'staged' | 'dispatched'>('active_rental');
  ...
  const openCreateModal = () => {
    if (!isAdmin) return;
    setEditingCase(null);
    setFormEquipmentCode(assets[0]?.assetCode || '');
    setFormStage('active_rental');
    setFormDestination('');
    setFormStartDate('');
    setFormEndDate('');
    setFormUserEmail('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      id: editingCase ? editingCase.id : `SCH-2026-${Date.now().toString().slice(-4)}`,
      equipmentCode: formEquipmentCode,
      model,
      sequenceOrder: editingCase ? editingCase.sequenceOrder : 0,
      stage: formStage,
    ...
    try {
      const url = editingCase ? `${API_BASE}/update` : `${API_BASE}/create`;
  ...
  const renderKanban = () => {
    const columns: ('active_rental' | 'calibration' | 'staged' | 'dispatched')[] = 
      ['active_rental', 'calibration', 'staged', 'dispatched'];
